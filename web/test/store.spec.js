...
import {dummySong, dummyUser} from './mocks/dummies.mock.js';
...
describe('store', () => {
    ...
    mockAdapter.onPost(`${handlerBasePath}/song/blah`).reply(404, {});
    ...
    mockAdapter.onPost(`${handlerBasePath}/user/blah`).reply(404, {});
    mockAdapter.onPost(`${handlerBasePath}/user/invalid-username`).reply(422, {
        "userCreated": false,
        "errorMessages": {
            "1010": "The username must be at least 4 characters."
        }
    });
    mockAdapter.onPost(`${handlerBasePath}/user/12345/delete`).reply(200, {});
    mockAdapter.onPost(`${handlerBasePath}/user/blah/delete`).reply(404, {});
    ...
    describe('actions', () => {
        describe('manager', () => {
            ...
            it('updateItemWithInvalidUsername', async () => {
                
                const user = dummyUser(null);
                const payload = { data: user, handler: 'user' };
                const createResponse = await createUser(payload);
                const updatedUser = dummyUser(createResponse.userId);
                updatedUser.username = 'nog';
                updatedUser.id = 'invalid-username';
                payload.data = updatedUser;

                let err = false;
                try {
                    await store.dispatch('manage/updateItem', payload);
                } catch (e) {
                    err = true;
                    expect(e.errorMessages).toEqual({ '1010': 'The username must be at least 4 characters.' })
                    expect(e.userCreated).toBeFalsy()
                } finally {
            ...
            it('updateItem', async () => {
                ...
                const createResponse = await createUser(payload);
                const updatedUser = dummyUser(createResponse.userId);
                updatedUser.lastName = 'Updated user title';
                payload.data = updatedUser;
        ...
        describe('catalog', () => { 
            ...
            it('updateItemNotFound', async () => {
                const payload = { data: dummySong('blah'), handler: 'song' };
                let err = false;
                try {
            ...
            it('deleteItemNotFound', async () => {
                const payload = { id: 'blah', handler: 'song' };
                let err = false;
                try {
                    await store.dispatch('deleteItem', payload);
                } catch (e) {
